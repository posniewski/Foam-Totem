...
/*     (c) Chris Veness 2002-2010: www.movable-type.co.uk/scripts/tea-block.html                  */
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
...
function decrypt() {
	...
	if(typeof(s.anchorNode) === "undefined") {
		// I think this is some IE thing
		var r = document.selection.createRange();
		r = r.parentElement();
		if(r.nodeName.toLowerCase() === "textarea") {
			var text = Tea.decrypt(p.value, password);
			p.value = text;
	...
	if(!done) {
		// See if we're inside a textarea
		var cs = s.anchorNode.children;
		if(typeof(cs) !== "undefined") {
			for(i=0; i<cs.length; i++) {
				if(cs[i].nodeType == 1) {
					if(cs[i].nodeName.toLowerCase() === "textarea") {
						var text = Tea.decrypt(cs[i].value, password);
						cs[i].value = text;
	...
	if(!done) {
		var text = Tea.decrypt(s.toString(), password);
